  ...
  def main(args: Array[String]) = {
    ...
    if (args.length < 4) {
      println("Usage: ProcessTweets " + "<input_tweets_directory> " +
        "<domains_file> <urls_file> <output_file> <collect_rel_bool> <consumer_key> <consumer_secret> <access_token> <access_token_secret>")
      System.exit(1)
    }
    ...
    var collect_relationships = args(4).toBoolean
    val consumerKey = args(5)
    val consumerSecret = args(6)
    val accessToken = args(7)
    val accessTokenSecret = args(8)
