...
def main(argv):
    ...
    preds={'attacker':0, 'malicious':0, 'verified':0, 'friend':0, 'isFollowedBy':0, 'mentions':0, 'retweeted':0, 'tweeted':0, 'isPossiblySensitive':0, 'containsHashtag':0, 'containsLink':0, 'followersCount':0, 'friendsCount':0, 'retweetCount':0, 'statusesCount':0, 'trending':0, 'favouritesCount':0}
    f = open(argv[0])
    for line in f:
        l = line.split('(',1)
        pred = l[0]
